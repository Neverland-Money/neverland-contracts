...
library DelegationLogicLibrary {
    ...
    function checkpointDelegator(
        mapping(uint256 => IDustLock.LockedBalance) storage _locked,
        mapping(uint256 => uint48) storage _numCheckpoints,
        mapping(uint256 => mapping(uint48 => IDustLock.Checkpoint)) storage _checkpoints,
        ...
        IDustLock.Checkpoint storage cpOld = numCheckpoint > 0
        ...
        IDustLock.Checkpoint storage cp = _checkpoints[_delegator][numCheckpoint];
    ...
    function checkpointDelegatee(
        ...
        IDustLock.Checkpoint storage cp = _checkpoints[_delegatee][numCheckpoint];
        cp.fromTimestamp = block.timestamp;
    ...
    function getPastVotesIndex(
        ...
        while (upper > lower) {
            ...
            IDustLock.Checkpoint storage cp = _checkpoints[_tokenId][center];
    ...
    function getPastVotes(
        ...
        IDustLock.Checkpoint memory lastCheckpoint = _checkpoints[_tokenId][_checkIndex];
        ...
        return
            lastCheckpoint.delegatee == 0
                ? votes + IDustLock(address(this)).balanceOfNFTAt(_tokenId, _timestamp)
                : votes;
    }
}
