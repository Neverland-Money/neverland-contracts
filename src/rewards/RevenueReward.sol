...
contract RevenueReward is IRevenueReward, ERC2771Context, ReentrancyGuard {
    ...
    function _getRewardUntilTsSingle(
        ...
        if (!dustLock.isApprovedOrOwner(sender, tokenId)) revert NotOwner();
    ...
    function enableSelfRepayLoan(uint256 tokenId, address rewardReceiver) external virtual override nonReentrant {
        _enableSelfRepayLoan(tokenId, rewardReceiver, _msgSender());
    }

    /// @inheritdoc IRevenueReward
    function enableSelfRepayLoanBatch(uint256[] calldata tokenIds, address rewardReceiver)
        external
        override
        nonReentrant
    {
        ...
        uint256 len = tokenIds.length;
        for (uint256 i = 0; i < len; i++) {
            _enableSelfRepayLoan(tokenIds[i], rewardReceiver, sender);
        }
    }
     ...
     * @param rewardReceiver The address that will receive the rewards (or zero address if disabled)
     * @param sender The address that called the function
     */
    function _enableSelfRepayLoan(uint256 tokenId, address rewardReceiver, address sender) internal {
