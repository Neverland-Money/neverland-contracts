...
contract DustLockValidationTest is BaseTest {
    ...
    function testComprehensivePrecisionValidation() public {
        ...
        for (uint256 i = 0; i < testCases.length; i++) {
            ...
            if (result.absoluteError == 0) {
                perfectMatches++;
            } else if (result.relativeErrorBasisPoints <= 1) { // <= 0.01%
                excellentPrecision++;
            } else if (result.relativeErrorBasisPoints <= 10) { // <= 0.1%
                goodPrecision++;
            }
        }
    ...
    function _testSinglePrecisionCase(TestCase memory test) internal returns (PrecisionResult memory) {
        ...
        uint256 relativeErrorBasisPoints = expectedVotingPower > 0 
            ? (absoluteError * 10000) / expectedVotingPower 
            : 0;
