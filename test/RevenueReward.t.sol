...
/**
 * @title RevenueRewardsTest
 * @notice Tests for revenue reward distribution system
 * @dev PRECISION TOLERANCE NOTE: After implementing PRB Math for DustLock voting power
 *      calculations, some reward distribution tests required slightly higher tolerances
 *      (±3 vs ±2). This is a positive indicator showing we now get mathematically correct
 *      results with natural rounding variations, instead of consistently wrong results
 *      from the old integer truncation system that were artificially precise to test.
 */
