...
contract DustLockVelodromeTest is BaseTest {
    ...
    function testVotingEscrowFlow() public {
        ...
        assertEq(dustLock.userPointEpoch(1), 1);
        ...
        assertEq(globalPoint.blk, 1);
        assertEq(globalPoint.permanentLockBalance, 0);
        globalPoint = dustLock.pointHistory(2);
        ...
        assertEq(locked.end, 126403200);
        assertEq(locked.isPermanent, false);
        assertEq(dustLock.slopeChanges(126403200), -23782343987);
        ...
        assertEq(userPoint.blk, 1);
        assertEq(userPoint.permanent, 0);
        userPoint = dustLock.userPointHistory(1, 2);
        ...
        assertEq(globalPoint.blk, 2);
        assertEq(globalPoint.permanentLockBalance, 0);
        // when checkpoints are skipped during epochs, dustLock attempts to
        ...
        // user points:         3
        // global point epoch:  5
        dustLock.increaseUnlockTime(1, MAXTIME);
        ...
        assertEq(dustLock.slopeChanges(127612800), -23782343987);

        assertEq(dustLock.userPointEpoch(1), 3);
        // check prior point
        userPoint = dustLock.userPointHistory(1, 2);
        ...
        assertEq(userPoint.blk, 2);
        assertEq(userPoint.permanent, 0);
        userPoint = dustLock.userPointHistory(1, 3);
        assertEq(userPoint.bias, 2991695181593523613); // slope * (127612800 - 1818001)
        assertEq(userPoint.slope, 23782343987);
        ...
        assertEq(globalPoint.blk, 10);
        assertEq(globalPoint.permanentLockBalance, 0);
        globalPoint = dustLock.pointHistory(6);
