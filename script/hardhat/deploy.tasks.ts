...
type AddressBook = Partial<Record<DeployableContract, string>>;
...
const requireConfigValue = (value: string | undefined, label: string): string => {
  if (!value || value.trim() === "") {
...
const isValidAddress = (value?: string): value is string => !!value && value.length === 42 && value.startsWith("0x");
...
const tryGetSigner = async (
  ...
  if (opts?.dryRun) {
    try {
      ...
      console.warn(`⚠️  Unable to impersonate signer for ${address}: ${(impErr as Error).message}`);
    }
  }
...
const formatEther = (hre: HardhatRuntimeEnvironment, wei: bigint): string => hre.ethers.formatEther(wei);
const formatGwei = (hre: HardhatRuntimeEnvironment, wei: bigint): string => hre.ethers.formatUnits(wei, "gwei");
...
const reportTx = async (
  ...
  const gasPrice: bigint = receipt.effectiveGasPrice ?? receipt.gasPrice ?? BigInt(0);
  const costWei = gasUsed * gasPrice;
  const from: string = (receipt.from ?? tx.from ?? "").toString();
  gasLog.push({ label, from, gasUsed, gasPrice, costWei });
  console.log(
    `   ⛽ ${label}: gas=${gasUsed.toString()} price=${formatGwei(hre, gasPrice)} gwei cost=${formatEther(
      hre,
      costWei,
    )} MON`,
  );
...
const reportDeployment = async (
  ...
  try {
    const depTx = typeof contract.deploymentTransaction === "function" ? contract.deploymentTransaction() : null;
    if (depTx) {
...
const deployNeverland = async (
  ...
  if (!dryRun) {
    console.log("🔧 Ensuring clean compilation for verification compatibility...");
  ...
  if (dryRun) {
    ...
    console.log("🚧 Dry-run mode: deploying to forked state under snapshot; will revert");
  }
  ...
  if (dryRun) {
    try {
      ...
      throw new Error("🚨 DRY-RUN SAFETY ERROR: Could not take snapshot. Dry-run requires a local forked network.");
    }
  }
  ...
  const requireExistingAddress = (name: DeployableContract, reason: string): string => {
    const value = configAddresses[name];
    if (!isValidAddress(value)) {
      throw new Error(`Missing or invalid address for ${name} ${reason}. Please provide it in config.addresses.${name}`);
    }
    return value;
  };
  ...
  const recordProxyInfo = async (name: DeployableContract, proxyAddr: string): Promise<void> => {
    try {
      const impl = await hre.upgrades.erc1967.getImplementationAddress(proxyAddr);
      implementations[name] = impl;
    } catch (e) {
      console.warn(`⚠️  Could not fetch implementation for ${name} at ${proxyAddr}`);
    }
    try {
      ...
      if (!proxyAdminAddress) proxyAdminAddress = admin;
        ...
        console.warn(`⚠️  Multiple ProxyAdmin addresses detected: ${proxyAdminAddress} vs ${admin}`);
      }
    } catch (e) {
  ...
  for (const name of exclude) {
    const addr = requireExistingAddress(name, "because it is excluded via --exclude.");
  ...
  if (!exclude.has("Dust")) {
    ...
    const totalSupplyRaw = requireConfigValue(config.dust?.totalSupply, "dust.totalSupply");
    const totalSupply = BigInt(totalSupplyRaw);

    console.log("\n⛏️  Deploying Dust (proxy)...");
    const dustFactory = await hre.ethers.getContractFactory("Dust");
    const dust = await hre.upgrades.deployProxy(dustFactory, [dustOwner, totalSupply], {
    ...
    const dustImpl = await hre.upgrades.erc1967.getImplementationAddress(dustAddress);
  ...
  if (!exclude.has("DustLock")) {
    const forwarder = requireConfigValue(config.dustLock?.forwarder, "dustLock.forwarder");
    const baseURI = requireConfigValue(config.dustLock?.baseURI, "dustLock.baseURI");
    const dustAddress = getRecordedAddress("Dust");

    console.log("\n⛏️  Deploying BalanceLogicLibrary...");
    const balanceLibFactory = await hre.ethers.getContractFactory("BalanceLogicLibrary");
    ...
    await reportDeployment(hre, balanceLib, "Deploy BalanceLogicLibrary", gasLog);
    ...
    const dustLockImpl = await hre.upgrades.erc1967.getImplementationAddress(dustLockAddress);
    await verifyContract(hre, dustLockImpl, [forwarder], "src/tokens/DustLock.sol:DustLock", dryRun);
    ...
    if (config.dustLock?.earlyWithdrawTreasury) {
      ...
      if (treasuryAddress.toLowerCase() !== currentTreasury.toLowerCase()) {
        console.log(`⚙️  Setting early withdraw treasury to ${treasuryAddress}...`);
    ...
    if (config.dustLock?.minLockAmount) {
      ...
      if (currentMinLock !== desiredMinLock) {
        console.log(`⚙️  Updating min lock amount to ${desiredMinLock.toString()}...`);
  ...
  if (!exclude.has("DustRewardsController")) {
    ...
    const controllerFactory = await hre.ethers.getContractFactory("DustRewardsController");
    ...
    await reportDeployment(hre, controller, "Deploy DustRewardsController (proxy)", gasLog);
    await recordProxyInfo("DustRewardsController", controllerAddress);

    // Verify DustRewardsController implementation
    const controllerImpl = await hre.upgrades.erc1967.getImplementationAddress(controllerAddress);
    await verifyContract(hre, controllerImpl, [emissionManager], "src/emissions/DustRewardsController.sol:DustRewardsController", dryRun);
  } else {
  ...
  if (!exclude.has("UserVaultRegistry")) {
    ...
    const executorAddress = requireConfigValue(registryConfig.executor, "selfRepaying.registry.executor");
    ...
    const registryFactory = await hre.ethers.getContractFactory("UserVaultRegistry");
    ...
    await reportDeployment(hre, registry, "Deploy UserVaultRegistry", gasLog);
      ...
      if (currentExecutor.toLowerCase() !== executorAddress.toLowerCase()) {
        console.log(`⚙️  Setting UserVaultRegistry executor to ${executorAddress}...`);
      ...
      if (currentMaxSlippage !== maxSlippage) {
        console.log(`⚙️  Updating max swap slippage to ${maxSlippageRaw} bps...`);
        const tx = await registry.setMaxSwapSlippageBps(maxSlippage);
        await reportTx(hre, tx, "UserVaultRegistry.setMaxSwapSlippageBps", gasLog);
      ...
      for (const aggregator of supportedAggregators) {
        ...
        if (!active) {
          ...
          await reportTx(hre, tx, `UserVaultRegistry.setSupportedAggregators(${aggregator})`, gasLog);
  ...
  if (!exclude.has("UserVaultImplementation")) {
    ...
    const userVaultImplFactory = await hre.ethers.getContractFactory("UserVault");
    ...
    console.log(`✅ UserVault implementation deployed at ${userVaultImplAddress}`);
    await reportDeployment(hre, userVaultImpl, "Deploy UserVault Implementation", gasLog);
  } else {
    const existingImplementation = getRecordedAddress("UserVaultImplementation");
    console.log(
  ...
  if (!exclude.has("UserVaultBeacon")) {
    ...
    const userVaultImplementationAddress = getRecordedAddress("UserVaultImplementation");

    console.log("\n⛏️  Deploying UserVault UpgradeableBeacon...");
    const beaconFactory = await hre.ethers.getContractFactory("UpgradeableBeacon");
    ...
    await reportDeployment(hre, beacon, "Deploy UserVault UpgradeableBeacon", gasLog);

    if (beaconOwner.toLowerCase() !== (await deployer.getAddress()).toLowerCase()) {
      console.log(`⚙️  Transferring beacon ownership to ${beaconOwner}...`);
      const beaconContract = await hre.ethers.getContractAt("UpgradeableBeacon", beaconAddress, deployer);
  ...
  if (!exclude.has("UserVaultFactory")) {
    ...
    const factoryFactory = await hre.ethers.getContractFactory("UserVaultFactory");
    const userVaultFactory = await hre.upgrades.deployProxy(
      factoryFactory,
      [],
      { initializer: false, kind: "transparent", unsafeAllow: ["missing-initializer"] },
    );
    ...
    console.log(`✅ UserVaultFactory (proxy) deployed at ${userVaultFactoryAddress}`);
    await reportDeployment(hre, userVaultFactory, "Deploy UserVaultFactory (proxy)", gasLog);
  ...
  if (!exclude.has("RevenueReward")) {
    const forwarder = requireConfigValue(config.revenueReward?.forwarder, "revenueReward.forwarder");
    const dustLockAddress = getRecordedAddress("DustLock");
    const rewardsControllerAddress = getRecordedAddress("DustRewardsController");
    ...
    const poolAddressesProviderRegistry = config.selfRepaying?.poolAddressesProviderRegistry;

    if (!exclude.has("UserVaultFactory")) {
      requireConfigValue(poolAddressesProviderRegistry, "selfRepaying.poolAddressesProviderRegistry");
    }

    console.log("\n⛏️  Deploying RevenueReward (proxy)...");
    const revenueRewardFactory = await hre.ethers.getContractFactory("RevenueReward");
    const revenueReward = await hre.upgrades.deployProxy(
      revenueRewardFactory,
      [forwarder, dustLockAddress, rewardsControllerAddress, userVaultFactoryAddress],
      {
        initializer: "initialize",
        kind: "transparent",
        constructorArgs: [forwarder],
    ...
    await reportDeployment(hre, revenueReward, "Deploy RevenueReward (proxy)", gasLog);
    await recordProxyInfo("RevenueReward", revenueRewardAddress);

    // Verify RevenueReward implementation
    const revenueRewardImpl = await hre.upgrades.erc1967.getImplementationAddress(revenueRewardAddress);
    await verifyContract(hre, revenueRewardImpl, [forwarder], "src/rewards/RevenueReward.sol:RevenueReward", dryRun);

    console.log("⚙️  Linking DustLock -> RevenueReward...");
    const dustLock = await hre.ethers.getContractAt("DustLock", dustLockAddress, deployer);
    ...
    if (!exclude.has("UserVaultFactory")) {
      ...
      if (!isValidAddress(poolRegistryAddress)) {
        throw new Error(`Invalid poolAddressesProviderRegistry address provided: ${poolRegistryAddress}`);
      }
      const userVaultFactory = await hre.ethers.getContractAt("UserVaultFactory", userVaultFactoryAddress, deployer);
      ...
      if (currentRegistry === hre.ethers.ZeroAddress) {
        ...
        console.log("ℹ️  UserVaultFactory already initialized; skipping initialization.");
      }
    }
  } else {
  ...
  if (!exclude.has("DustLockTransferStrategy")) {
    const rewardsAdmin = requireConfigValue(config.transferStrategy?.rewardsAdmin, "transferStrategy.rewardsAdmin");
    const dustVaultWallet = getEnvWallet(hre, "DUST_VAULT_PRIVATE_KEY");
    const dustVault = dustVaultWallet ? await dustVaultWallet.getAddress() : requireConfigValue(config.transferStrategy?.dustVault, "transferStrategy.dustVault");
    const dustLockAddress = getRecordedAddress("DustLock");
    const controllerAddress = getRecordedAddress("DustRewardsController");
    const incentivesController = config.transferStrategy?.incentivesControllerOverride ?? controllerAddress;
    const emissionManager = deployerAddress;
    const dustAddress = getRecordedAddress("Dust");
    console.log("\n⛏️  Deploying DustLockTransferStrategy...");
    const strategyFactory = await hre.ethers.getContractFactory("DustLockTransferStrategy");
    const strategy = await strategyFactory.deploy(incentivesController, rewardsAdmin, dustVault, dustLockAddress);
    ...
    await reportDeployment(hre, strategy, "Deploy DustLockTransferStrategy", gasLog);
    ...
    const emissionManagerSigner =
      emissionManager.toLowerCase() === (await deployer.getAddress()).toLowerCase()
        ? deployer
        : await tryGetSigner(hre, emissionManager, { dryRun });

    if (emissionManagerSigner) {
      console.log("⚙️  Registering transfer strategy with DustRewardsController...");
      try {
        const tx = await (rewardsController as any)
          .connect(emissionManagerSigner)
          .setTransferStrategy(dustAddress, strategyAddress);
        await reportTx(hre, tx, "DustRewardsController.setTransferStrategy", gasLog);
        console.log("✅ Transfer strategy registered.");
      } catch (err) {
        console.warn("⚠️  Could not register transfer strategy now. Deferring as a pending action.");
        pendingActions.push(
    ...
    if (currentAllowance === hre.ethers.MaxUint256) {
      console.log("✅ DUST vault already approved strategy with MaxUint allowance.");
    } else {
      const vaultSigner = dustVaultWallet
        ? dustVaultWallet
        : dustVault.toLowerCase() === (await deployer.getAddress()).toLowerCase()
          ? deployer
          : await tryGetSigner(hre, dustVault, { dryRun });
      if (vaultSigner) {
        console.log("⚙️  Approving DUST vault allowance for strategy...");
        try {
          const approveTx = await (dust.connect(vaultSigner) as any).approve(strategyAddress, hre.ethers.MaxUint256);
          await reportTx(hre, approveTx, "DUST.approve(MaxUint256)", gasLog);
          const updatedAllowance = await dust.allowance(dustVault, strategyAddress);
          ...
          console.warn("⚠️  Could not set DUST vault approval now. Deferring as a pending action.");
          pendingActions.push(
  ...
  if (!exclude.has("NeverlandDustHelper")) {
    ...
    if (uniswapPair && !isValidAddress(uniswapPair)) {
      throw new Error(`Invalid dustHelper.uniswapPair address provided: ${uniswapPair}`);
    }

    console.log("\n⛏️  Deploying NeverlandDustHelper...");
    const helperFactory = await hre.ethers.getContractFactory("NeverlandDustHelper");
    ...
    await verifyContract(hre, helperAddress, [dustAddress, owner], "src/helpers/NeverlandDustHelper.sol:NeverlandDustHelper", dryRun);

    if (uniswapPair) {
      const currentOwner = await helper.owner();
      const desiredPair = uniswapPair;
      const ownerSigner = currentOwner.toLowerCase() === (await deployer.getAddress()).toLowerCase()
        ? deployer
        : await tryGetSigner(hre, currentOwner, { dryRun });

      if (ownerSigner) {
        const helperWithOwner = helper.connect(ownerSigner);
        console.log(`⚙️  Setting NeverlandDustHelper Uniswap pair to ${desiredPair}...`);
        try {
          ...
          console.warn("⚠️  Could not set Uniswap pair now. Deferring as a pending action.");
          pendingActions.push(
  ...
  if (!exclude.has("NeverlandUiProvider")) {
    ...
    const uiFactory = await hre.ethers.getContractFactory("NeverlandUiProvider");
    ...
    await reportDeployment(hre, uiProvider, "Deploy NeverlandUiProvider", gasLog);

    // Verify NeverlandUiProvider
    await verifyContract(hre, uiAddress, [dustLockAddress, revenueRewardAddress, controllerAddress, dustOracleAddress, aaveProvider], "src/ui/NeverlandUiProvider.sol:NeverlandUiProvider", dryRun);
  } else {
  ...
  const summaryRows = ALL_CONTRACTS.map((name) => ({
    contract: name,
    address: (addresses[name] ?? configAddresses[name] ?? "(not deployed / not provided)") as string,
  }));
  ...
  try {
    ...
    const deploymentsRoot = path.resolve(process.cwd(), "deployments", networkName);
    ...
    fs.writeFileSync(path.join(deploymentsRoot, "addresses.json"), JSON.stringify(addressesJson, null, 2));

    const writtenRows: Array<{ contract: string; file: string }> = [];

    // Helper: resolve Hardhat metadata JSON for a contract by name
    const getHardhatMetadata = async (contractName: string): Promise<any | null> => {
      try {
        // Try via artifacts to learn the sourceName then fetch build info
        const artifact = await hre.artifacts.readArtifact(contractName as string);
        const fqName = `${artifact.sourceName}:${artifact.contractName}`;
        const buildInfo = await hre.artifacts.getBuildInfo(fqName);
        if (buildInfo) {
          const metaStr = (buildInfo as any).output?.contracts?.[artifact.sourceName]?.[artifact.contractName]?.metadata;
        ...
        try {
          ...
          for (const f of files) {
            ...
            for (const [src, table] of Object.entries(contracts) as Array<[string, any]>) {
    ...
    for (const [name, addr] of Object.entries(addresses)) {
      ...
      if (!metaObj) {
        console.warn(`⚠️  Metadata not found for ${name} in artifacts/build-info`);
      }
      const isProxied = implementations[name as DeployableContract] !== undefined;
      const content: any = {
        networkName,
        chainId,
        contract: name,
      ...
      writtenRows.push({ contract: name, file: `deployments/${networkName}/${name}.json` });
    }
    ...
    if (proxyAdminAddress) {
      ...
      try {
        ...
        const biDir = path.resolve(process.cwd(), "artifacts", "build-info");
        const files = fs.existsSync(biDir) ? fs.readdirSync(biDir).filter((f) => f.endsWith('.json')) : [];
        for (const f of files) {
          const bi = JSON.parse(fs.readFileSync(path.join(biDir, f), 'utf8'));
          const entry = bi.output?.contracts?.["@openzeppelin/contracts/proxy/transparent/ProxyAdmin.sol"]?.ProxyAdmin;
          if (entry?.metadata) { adminMeta = JSON.parse(entry.metadata); break; }
        }
      } catch (_) {}
      const adminContent: any = { networkName, chainId, contract: "ProxyAdmin", proxyAdmin: proxyAdminAddress };
      if (adminMeta) adminContent.metadata = adminMeta;
      fs.writeFileSync(adminFile, JSON.stringify(adminContent, null, 2));
      writtenRows.push({ contract: "ProxyAdmin", file: `deployments/${networkName}/ProxyAdmin.json` });
    }
    ...
    console.warn("⚠️  Could not write deployments folder:", (e as Error).message);
  }
  ...
  if (gasLog.length > 0) {
    ...
    gasRows.push({ step: "TOTAL", from: "", gasUsed: totalGas.toString(), gasPriceGwei: "", costMON: formatEther(hre, totalCostWei) });
    ...
    if (firstGasPrice > 0n) {
      ...
      gasRows.push({ step: `TOTAL@FIRST(${formatGwei(hre, firstGasPrice)} gwei)`, from: "", gasUsed: totalGas.toString(), gasPriceGwei: formatGwei(hre, firstGasPrice), costMON: formatEther(hre, projectedFirstWei) });
    }
    const budgetGwei = process.env.BUDGET_GAS_GWEI ? BigInt(Math.floor(parseFloat(process.env.BUDGET_GAS_GWEI) * 1e9)) : 0n;
    if (budgetGwei > 0n) {
      const projectedEnvWei = totalGas * budgetGwei;
      gasRows.push({ step: `TOTAL@ENV(${process.env.BUDGET_GAS_GWEI} gwei)`, from: "", gasUsed: totalGas.toString(), gasPriceGwei: process.env.BUDGET_GAS_GWEI!, costMON: formatEther(hre, projectedEnvWei) });
    }
  ...
  console.table(reminders.map((text, idx) => ({ item: idx + 1, reminder: text })));
...
task("deploy:neverland", "Deploy the complete Neverland protocol")
  ...
  .addOptionalParam("exclude", "Comma-separated list of contracts to exclude from deployment", "")
  .addFlag("dryRun", "Execute a dry run to preview deployment without making changes")
  .setAction(async (taskArgs: TaskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    if (taskArgs.exclude) {
      const items = taskArgs.exclude.split(",").map((item) => item.trim()).filter(Boolean);
  ...
  .setAction(async (taskArgs: TaskArgs) => {
    ...
    try {
      ...
      const requiredByContract: Record<DeployableContract, string[]> = {
        ...
        DustRewardsController: [],
        UserVaultRegistry: ["selfRepaying.registry.executor", "selfRepaying.registry.maxSwapSlippageBps"],
        UserVaultImplementation: [],
        UserVaultBeacon: [],
      ...
      if (missing.length > 0) {
        ...
        console.log("✅ All required fields present for non-excluded contracts");
      }
      ...
      if (!exclude.has("DustLockTransferStrategy")) {
        const hasVaultKey = !!process.env.DUST_VAULT_PRIVATE_KEY && process.env.DUST_VAULT_PRIVATE_KEY.trim() !== "";
        const cfgVault = config.transferStrategy?.dustVault;
        if (!hasVaultKey && (!cfgVault || cfgVault.trim() === "")) {
          console.log("❌ Missing DUST vault: provide DUST_VAULT_PRIVATE_KEY env or transferStrategy.dustVault in config.");
        }
      }

      // Ensure excluded contracts have provided addresses
      const invalidExcluded: string[] = [];
      const isValid = (value?: string) => !!value && value.length === 42 && value.startsWith("0x");
      for (const name of exclude) {
      ...
      if (invalidExcluded.length > 0) {
        console.log("❌ Excluded contracts missing addresses in config.addresses:");
