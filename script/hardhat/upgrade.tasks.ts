...
task("deployImpl:DustLock", "Deploy new DustLock implementation")
  ...
  .setAction(async (args, hre) => {
    const { forwarder, balancelib } = args as {
      forwarder: string;
      balancelib: string;
    };
...
task("deployImpl:RevenueReward", "Deploy new RevenueReward implementation")
  ...
  .setAction(async (args, hre) => {
    const { forwarder, dryrun } = args as {
      forwarder: string;
      dryrun: boolean;
    };
    ...
    console.log("RevenueReward impl:", implAddress);
...
task("upgrade:proxy", "Upgrade a TransparentUpgradeableProxy via ProxyAdmin")
  ...
  .setAction(async (args, hre) => {
    const { proxyadmin, proxy, impl } = args as {
      proxyadmin: string;
      proxy: string;
      impl: string;
    };
...
async function showImplChange(
  ...
  if (before.toLowerCase() === after.toLowerCase()) {
    console.warn(
      "⚠️  Implementation unchanged. Ensure code diff or use redeployImplementation option."
    );
  }
}
...
task("upgrade:dust", "Upgrade Dust proxy using a fresh implementation")
  .addOptionalParam(
    "proxy",
    "Dust proxy address (defaults to config.addresses.Dust)"
  )
  .addOptionalParam(
    "configFile",
  ...
  .setAction(async (args: { proxy?: string; configFile: string }, hre) => {
    ...
    if (!proxy)
      throw new Error(
        "Missing Dust proxy address (pass --proxy or set config.addresses.Dust)"
      );
...
task(
  "upgrade:dustlock",
  "Upgrade DustLock proxy (requires --forwarder and --balancelib)"
)
  .addOptionalParam(
    "proxy",
    "DustLock proxy address (defaults to config.addresses.DustLock)"
  )
  .addParam("forwarder", "Trusted forwarder address")
  .addParam("balancelib", "Deployed BalanceLogicLibrary address")
  .addOptionalParam(
  ...
  .setAction(
    async (
      ...
      if (!proxy)
        throw new Error(
          "Missing DustLock proxy address (pass --proxy or set config.addresses.DustLock)"
        );
      console.log(`Upgrading DustLock at ${proxy}...`);
      const F = await hre.ethers.getContractFactory("DustLock", {
        libraries: { BalanceLogicLibrary: args.balancelib },
      });
...
//////////////////////////////////////////////////////////////*/
task(
  "upgrade:revenuereward",
  "Upgrade RevenueReward proxy (requires --forwarder)"
)
  .addOptionalParam(
    "proxy",
    "RevenueReward proxy address (defaults to config.addresses.RevenueReward)"
  )
  .addParam("forwarder", "Trusted forwarder address")
  .addOptionalParam(
    "configFile",
  ...
  .setAction(
    async (
      args: { proxy?: string; forwarder: string; configFile: string },
      hre
    ) => {
      const config = loadConfig(args.configFile);
      ...
      if (!proxy)
        throw new Error(
          "Missing RevenueReward proxy address (pass --proxy or set config.addresses.RevenueReward)"
        );
...
//////////////////////////////////////////////////////////////*/
task(
  "upgrade:dustrewardscontroller",
  "Upgrade DustRewardsController proxy (requires --emissionmanager)"
)
  .addOptionalParam(
    "proxy",
    "DustRewardsController proxy address (defaults to config.addresses.DustRewardsController)"
  )
  ...
  .setAction(
    async (
      args: { proxy?: string; emissionmanager: string; configFile: string },
      hre
    ) => {
      const config = loadConfig(args.configFile);
      ...
      if (!proxy)
        throw new Error(
          "Missing DustRewardsController proxy address (pass --proxy or set config.addresses.DustRewardsController)"
        );
...
task("upgrade:uservaultfactory", "Upgrade UserVaultFactory proxy")
  .addOptionalParam(
    "proxy",
    "UserVaultFactory proxy address (defaults to config.addresses.UserVaultFactory)"
  )
  .addOptionalParam(
    "configFile",
    "Path to deployment config JSON file",
  ...
  .setAction(async (args: { proxy?: string; configFile: string }, hre) => {
    ...
    if (!proxy)
      throw new Error(
        "Missing UserVaultFactory proxy address (pass --proxy or set config.addresses.UserVaultFactory)"
      );
